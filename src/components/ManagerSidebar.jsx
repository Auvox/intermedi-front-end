...
function SidebarIcon({ name }) {
  const paths = {
    ...
    heart: <><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1.1-1.1a5.5 5.5 0 0 0-7.8 7.8L12 21l8.8-8.6a5.5 5.5 0 0 0 0-7.8Z" /><path d="M6 12h3l2-3 2 6 2-3h3" /></>,
      ...
      <nav aria-label="Menu do gerente">
        {links.map(({ path, label, icon }) => <NavLink key={path} end={!path} to={`/gerente${path ? `/${path}` : ''}`}>
          ...
          {path === 'chamados' && external > 0
            ? <span className="manager-sidebar-count" aria-label={`${external} chamados de outras farmácias`}>{external}</span>
            : <span className="manager-sidebar-chevron"><SidebarIcon name="arrow" /></span>}
        </NavLink>)}
      </nav>
    </div>
